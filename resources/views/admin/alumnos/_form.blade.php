@php
    ...
    $padreId = old('padre_id', $alumno?->padre_id);
@endphp
...
>
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div>
            ...
            <x-text-input id="contacto_emergencia" class="mt-1 block w-full" type="text" name="contacto_emergencia" :value="old('contacto_emergencia', $alumno?->contacto_emergencia)" required />
    ...
    <div>
        <x-input-label for="padre_id" value="Padre de familia" />
        <select
            id="padre_id"
            name="padre_id"
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
        >
            ...
            @foreach ($padres as $padreOpcion)
                <option value="{{ $padreOpcion['id'] }}" @selected((string) $padreId === (string) $padreOpcion['id'])>
                    {{ $padreOpcion['nombre'] }}
                </option>
            @endforeach
        </select>
        <p class="mt-1 text-xs text-slate-500">Opcional. Solo aparecen padres de este establecimiento.</p>
        <x-input-error class="mt-2" :messages="$errors->get('padre_id')" />
    </div>
    ...
    <div class="space-y-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
        <div>
            ...
            <p class="mt-1 text-xs text-slate-500">Se crea una cuenta con rol Alumno para que pueda iniciar sesión.</p>
        </div>
    ...
    <div>
        <label class="flex items-center gap-2 text-sm text-slate-700">
            ...
            <input
                ...
                @checked(old('activo', $alumno?->activo ?? 1))
            >
            Activo
        </label>
